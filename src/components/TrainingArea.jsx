...
function TrainingArea({ area, idx, onChange, removeArea, error, cleanError }) {
  return (
    <div className="space-y-4 border border-gray-300 rounded-lg p-4">
      ...
      <div className="grid grid-cols-[3fr_1fr_3fr] items-center mb-2">
        <div className="">
          <p className="font-bold mb-2">Width</p>
          <div className="join w-full">
            <button
              type="button"
              className="btn join-item"
              onClick={() => {
                onChange(area.id, { sizeX: Math.max(0, area.sizeX - 1) });
                cleanError("sizes");
              }}
            >
              -
        ...
        <div className="justify-self-center self-end p-2 text-lg font-bold">
          X
        </div>
        ...
        <div className="">
          ...
          <div className="join w-full">
            <button
              ...
              className="btn join-item"
              onClick={() => {
                onChange(area.id, { sizeY: Math.max(0, area.sizeY - 1) });
                cleanError("sizes");
              }}
            >
              -
      ...
      {error.sizes?.[area.id] && <ErrorAlert msg={error.sizes[area.id]} />}
    </div>
  );
}
